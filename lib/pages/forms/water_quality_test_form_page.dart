// lib/pages/forms/water_quality_test_form_page.dart
...
class WaterQualityTestFormPage extends StatefulWidget {
  ...
  static Future<Record?> addWaterQualityTest(
    ...
    if (isBottomSheet) {
      ...
      final result = await WaterQualityTestFormPage(
      ...
      // 页面模式：正常页面跳转
      final targetItem = await WaterQualityTestFormPage(
        record: preconfiguredRecord,
        aquariumId: aquariumId,
  ...
  @override
  _WaterQualityTestFormPageState createState() =>
      _WaterQualityTestFormPageState();
}

class _WaterQualityTestFormPageState extends State<WaterQualityTestFormPage> {
  // 输入辅助器
  ...
  void _handleSave() {
    ...
    // 构建记录名称
    String name = '水质测试 - ${_nameHelper.text.trim()}';
    ...
    final newRecord = Record(
      ...
      operationType: OperationType.statusChange,
      name: name,
      description: '水质测试: ${_nameHelper.text.trim()}',
