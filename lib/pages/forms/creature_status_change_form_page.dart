// lib/pages/forms/creature_status_change_form_page.dart
...
class CreatureStatusChangeFormPage extends StatefulWidget {
  ...
  static Future<Record?> addCreatureStatusChange(
    ...
    if (isBottomSheet) {
      ...
      final result = await CreatureStatusChangeFormPage(
      ...
      // 页面模式：正常页面跳转
      final targetItem = await CreatureStatusChangeFormPage(
        record: preconfiguredRecord,
        aquariumId: aquariumId,
    ...
    if (_sourceIdHelper.text.isNotEmpty) {
      ...
      if (sourceId == null) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('关联生物ID必须是整数')));
        return;
      }
    }
