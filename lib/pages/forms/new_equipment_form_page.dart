// lib/pages/forms/new_equipment_form_page.dart
...
class NewEquipmentFormPage extends StatefulWidget {
  final Record? record; // 传入 null 表示新建，否则编辑
  final int? aquariumId; // 可选：预填充的鱼缸ID
  final void Function(Record?)? onSave; // Modal模式下的保存回调

  NewEquipmentFormPage({Key? key, this.record, this.aquariumId, this.onSave})
    : super(key: key);
  ...
  static Future<Record?> addNewEquipment(
    ...
    if (isBottomSheet) {
      ...
      final result = await NewEquipmentFormPage(
      ...
      // 页面模式：正常页面跳转
      final targetItem = await NewEquipmentFormPage(
        record: preconfiguredRecord,
        aquariumId: aquariumId,
  ...
  @override
  _NewEquipmentFormPageState createState() => _NewEquipmentFormPageState();
}

class _NewEquipmentFormPageState extends State<NewEquipmentFormPage> {
  // 输入辅助器
  ...
  void _handleSave() {
    ...
    // 构建记录名称
    String name = '新购设备: ${_nameHelper.text.trim()}';
